...
if ( $CommitMessage -match '^doc:(\s|\S)*$') {
  ...
  Write-Output "::notice::The latest commit: $CommitMessage indicates a fix change. Merging this pull-request will increase the minor (0.0.x) version number."
  exit 0
} elseif ( $CommitMessage -match '^feat:(\s|\S)*$' ) {
  Write-Output "::notice::The latest commit: $CommitMessage indicates a non-breaking feature change. Merging this pull-request will increase the middle (0.x.0) version number."
  exit 0
} elseif ( $CommitMessage -match '^feat!:(\s|\S)*$' ) {
  Write-Output "::notice::The latest commit: $CommitMessage indicates a breaking feature change. Merging this pull-request will increase the major (x.0.0) version number."
  exit 0
} else {
